	...
	const stream = new ReadableStream({
		...
		start(controller) {
			...
			webSocketServer.addEventListener('message', (event) => {
				...
				// 将消息加入流的队列中
                // 修正"TransformStream is being used as a byte stream..."错误
                ...
                if (message instanceof ArrayBuffer) {
                    ...
                    // 如果是其他类型，尝试转换或忽略
                    // console.warn("Received unexpected message type:", typeof message);
                }
			});
			...
			if (error) {
                ...
                // 确保数据转换为 Uint8Array
                if (earlyData instanceof ArrayBuffer) {
                    controller.enqueue(new Uint8Array(earlyData));
                } else if (typeof earlyData === 'string') {
                    controller.enqueue(new TextEncoder().encode(earlyData));
                } else {
                    // 如果是其他类型，尝试转换或忽略
                    // console.warn("Received unexpected message type:", typeof earlyData);
                }
